...
exports.handler = async (event, context) => {
  ...
  if (method === "PATCH") {
    let data;
    try { data = JSON.parse(event.body); } catch (e) { return err(400, "JSON inválido"); }
    if (!data.code) return err(400, "code é obrigatório");
    const cert = await store.get(data.code, { type: "json" });
    if (!cert) return err(404, "Certificado não encontrado");
    if (data.status) cert.status = data.status;
    if (typeof data.studentEmail === "string") cert.studentEmail = data.studentEmail.trim();
    if (typeof data.cpf === "string") cert.cpf = data.cpf.trim();
